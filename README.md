...
"**Blast! Expense Tracker**" is my CS50 Python project. It's an expense tracker app that uses *CustomTKinter* for the interface. The application uses two primary files, *etracker.py* and *expenseDB.py* to implement 3 classes, *ExpenseTracker*, *TransForm* and *Expenses* to build its functionalities.

------------------------------

#### Note

Files, *project.py* and *test_project.py* are implemented to get auto-checker of CS50 to work and enable the submission of the project. They do not add to the actual functionality of the application. Project.py can be used to run the app.
...
By default it shows current month's transactions in the transaction list, but users can change dates to see a larger list. The summary also shows the current month's summary.

# Architecture

The expense tracker uses 3 classes:

- **ExpenseTracker**: This class is the primary application that creates the expense tracker, along with the database functions.
- **Expenses**: This class auto-creates the SQLite3 database required by the app.
...
- **build_form_ui()**: This function builds the UI of the form. The Trans_Form class is a subclass of *CTkFrame*, so it can be displayed on any GUI element that allows CTkFrame. The form UI has fields for date, type, category, amount, and description. It also has two buttons to save or cancel the transaction.
- **validate_form()**: This function validates all the data the user puts in the fields. If any data is missing or incorrect, it alerts users using an alert box, implemented using *CTkMessagebox*.
...
- **reset_form()**: Removes all data from the form fields and resets it back to its default setting. Called by default after a transaction is saved or cancelled.
...
- **get_transaction()**: The function allows the app to get a specific transaction, by searching by its id on the database.
...
- **update_transaction()**: The function allows user to update any transaction, by searching it with its id. The transaction is first fetched using *get_transaction* and populated to the form on the app. After user updates details and clicks save, the details are validated (within Trans_Form) and transaction is updated on the database.

- **del_transaction()**: Allows users to click the *delete* button and delete a specific transaction.

- **summary_generator()**: This function returns the summary of the month. It returns a dictionary with income, expense, and balance for the month. It can be used to generate summary for different periods by supplying different dates (although that functionality is not implemented in the interface).
...
The app uses *sqlite3* for the database. It's imported in *Expenses* class to create the database. The class encapsulated all required functions within the database. The Expenses object within the ExpenseTracker object is able to manipulate the data in the database directly from the app.
...
The plan for the future is to move Blast! Expense Tracker to a better GUI by implementing Qt for Python. CustomTKinter is good for modern UIs, but is still limited in its capabilities. In order to implement more features, such as reporting and exporting of transaction data, we need a powerful GUI building library. This is the plan for the future. In addition, I will be implementing more functionalities such as a dashboard, more options, a simplified transaction form that can display thousands of transactions at once without lag, exporting transaction data in various formats (CSV, PDF, etc.)
